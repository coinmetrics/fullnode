...
**NOTE**: This has a **big drawback** where release scripts modify the repo code before archiving, so the source code from the repo will be different to what is archived and will result in different hashes as the inputs are different.

in src/clientversion.cpp
```
...
```
This can be see in the source code of [btc](https://github.com/bitcoin/bitcoin/blob/master/src/clientversion.cpp#L33) and [dgb](https://github.com/DigiByte-Core/digibyte/blob/develop/src/clientversion.cpp#L28) to name some assets.
...
The source code tag archive can be downloaded and hash generated via the following command and note that the hash is different to `nix-prefetch-git` for the reason or release script modifications of the code. This is the same hash that will be generated by `nix-prefetch-github` and `nix-build` which do the download automatically... see nextion section
...
There is also a 3rd party lib [nix-prefetch-github](https://github.com/seppeljordan/nix-prefetch-github) which will download the .tar.gz archive for the tag. Its esentially a wrapper for `nix-build` with a 0 hash specified. This approach would be used over `nix-prefetch-git` when a release script modifies the repo code before archiving and for supporting keepDotGit or fetchSubmodules.
